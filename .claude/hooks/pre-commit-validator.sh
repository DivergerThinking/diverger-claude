...
if [ -f "package.json" ] && [ -f "plugin/.claude-plugin/plugin.json" ]; then
  VERSIONS=$(node -e "try{const fs=require('fs');const p=JSON.parse(fs.readFileSync('package.json','utf8')).version||'';const pl=JSON.parse(fs.readFileSync('plugin/.claude-plugin/plugin.json','utf8')).version||'';console.log(p+'\n'+pl)}catch{console.log('\n')}")
  PKG_VERSION=$(echo "$VERSIONS" | head -1)
  PLUGIN_VERSION=$(echo "$VERSIONS" | tail -1)
