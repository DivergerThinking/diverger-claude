    ...
    hooks: [
      ...
      },
      {
        ...
        hooks: [
          {
            type: 'command',
            command: 'FILE_PATH=$(node -e "try{console.log(JSON.parse(require(\'fs\').readFileSync(0,\'utf8\')).tool_input?.file_path||\'\')}catch{console.log(\'\')}"); [ -n "$FILE_PATH" ] && node -e "const f=process.argv[1]||\'\';if(!/\\.env/.test(f)||/\\.example/.test(f))process.exit(0);if(!/fastlane/i.test(f))process.exit(0);const c=require(\'fs\').readFileSync(f,\'utf8\');const issues=[];if(/(MATCH_PASSWORD|FASTLANE_.*PASSWORD|SUPPLY_JSON_KEY)\\s*=\\s*[^\\s$]/.test(c))issues.push(\'CRITICAL: Secret value in Fastlane .env file — use CI secrets manager instead of committing this file\');issues.forEach(i=>console.log(i))" -- "$FILE_PATH"',
            timeout: 5,
          },
        ],
      },
